...
class Winauto:
    ...
    def __init__(self, constructor, master, width=848, height=400):
        ...
        # Initiate the pop running flag. This flag is used to make the ui more fluid by preventing the
        # adcloop function from executing
        ...
        # Initiate some toggles to identify the state of the power relays (TODO: is this the best way to do it?)
        ...
        # Constant holding the desired loading time before running the first system check by using the inits_fn() [ms]
        ...
        self.feed_level_state = self.constructor.db['feed_level_state'] # Used for the interrupt algorithm in adcloop()
        # This list stores the last 4 values of feed_level_percent. The idea is to switch the feed level state if these 
        # four values are equal and different from the current state. In this way, oscillation is prevented (at least minimized).
        self.feed_level_lst = self.constructor.db['feed_level_lst']

        # Proceed flags: Used in semi-auto mode when it is asked to the user if he/she wants to proceed despite some undesired 
        # state of the system
        self.proceed_id = 1
        ...
        self.ima_feedpmp_off = tk.PhotoImage(master=self.master, data=graph.feedpmp_off)
        self.ima_feedpmp_on = tk.PhotoImage(master=self.master, data=graph.feedpmp_on)
        ...
        self.ima_outpmp_off = tk.PhotoImage(master=self.master, data=graph.outpmp_off)
        self.ima_outpmp_on = tk.PhotoImage(master=self.master, data=graph.outpmp_on)
        self.ima_reactor00 = tk.PhotoImage(master=self.master, data=graph.reactor00)
        self.ima_reactor01 = tk.PhotoImage(master=self.master, data=graph.reactor01)
        self.ima_reactor10 = tk.PhotoImage(master=self.master, data=graph.reactor10)
        self.ima_reactor11 = tk.PhotoImage(master=self.master, data=graph.reactor11)
        self.ima_recirpmp_off = tk.PhotoImage(master=self.master, data=graph.recirpmp_off)
        self.ima_recirpmp_on = tk.PhotoImage(master=self.master, data=graph.recirpmp_on)
        ...
        self.ima_feed0 = tk.PhotoImage(master=self.master, data=graph.ima_feed0)
        self.ima_feed1 = tk.PhotoImage(master=self.master, data=graph.ima_feed1)
        self.ima_o2_recir00 = tk.PhotoImage(master=self.master, data=graph.ima_o2_recir00)
        self.ima_o2_recir01 = tk.PhotoImage(master=self.master, data=graph.ima_o2_recir01)
        self.ima_o2_recir10 = tk.PhotoImage(master=self.master, data=graph.ima_o2_recir10)
        self.ima_o2_recir11 = tk.PhotoImage(master=self.master, data=graph.ima_o2_recir11)
        ...
        self.lbl_feedpmp = tk.Label(self.master, image=self.ima_feedpmp_off, bd=0)
        ...
        self.lbl_outpmp = tk.Label(self.master, image=self.ima_outpmp_off, bd=0)
        self.lbl_reactor = tk.Label(self.master, image=self.ima_reactor00, bd=0)
        self.lbl_recirpmp = tk.Label(self.master, image=self.ima_recirpmp_off, bd=0)
        self.lbl_uv = tk.Label(self.master, image=self.ima_uv_off, bd=0)
        self.lbl_shut = tk.Label(self.master, image=self.ima_shut, bd=0)
        self.lbl_level_percent = tk.Label(self.master, text='', bg='white', bd=0,
                                          fg=FG_COLOR, font=('Roboto', 10))

        # Info textbox
        self.finfo = tk.Frame(self.master, width=300, height=90)
        self.finfo.pack_propagate(0) # Don't shrink
        self.finfo.place(x=44, y=247)
        self.linfo = tk.Message(self.finfo, bg='white', text=self.LINFO_ISTRING, bd=0,
                              fg=FG_COLOR, anchor=tk.N, font=('Roboto', 12),
                              justify=tk.CENTER, width=250) # font=FONT_INFO,
        ...
        self.linfo_kw = tk.Label(self.finfo_kw, bg='white', text=self.LINFO_KW_ISTRING,
                                 bd=0, fg=FG_COLOR, anchor=tk.S,
                                 font=('Roboto', 12, 'bold')) # font=FONT_INFO_KW,
        self.linfo_kw.pack(fill=tk.BOTH, expand=1)

        self.finfo_lblbtn = tk.Frame(self.master, width=300, height=40, bg='white')
        self.finfo_lblbtn.pack_propagate(0) # Don't shrink
        self.finfo_lblbtn.place(x=44, y=337)

        self.btn_start_steri = tk.Label(self.finfo_lblbtn, text='START STERI.', fg='white', bg=LIGHT_BLUE_COLOR, font=('Roboto', 11))
        self.btn_start_steri.pack(ipady=5, ipadx=5, pady=3)
        self.btn_start_steri.pack_forget()

        self.btn_proceed = tk.Label(self.finfo_lblbtn, text='PROCEED', fg='white', bg=LIGHT_BLUE_COLOR, font=('Roboto', 11))
        ...
        self.w = tk.Canvas(self.master, width=412, height=280, bg=BG_COLOR, highlightthickness=0)
        self.w.place(x=392, y=102)

        self.canvas_out = self.w.create_image(295, 0, image=self.ima_out0, anchor=tk.NW)
        self.canvas_uv = self.w.create_image(341, 10, image=self.ima_uv0, anchor=tk.NW)
        self.canvas_o2_recir = self.w.create_image(0, 0, image=self.ima_o2_recir00, anchor=tk.NW)
        self.canvas_feed = self.w.create_image(0, 210, image=self.ima_feed0, anchor=tk.NW)
        self.canvas_o3 = self.w.create_image(130, 10, image=self.ima_o30, anchor=tk.NW)
        ...
        self.feedpmp = self.w.create_polygon(self.x0_feedpmp, self.y0_feedpmp,
                                             ...
                                             width=3, fill=BG_COLOR, outline=FG_COLOR,
                                             tags=('filling_poly', ))
        ...
        self.feedlevel = self.w.create_rectangle(self.canvas_feedlevel_x + 1,
                                                 ...
                                                 width=2, fill=GREEN_COLOR, outline=GREEN_COLOR) # (x=12, y=151, w=76, h=117)

        self.reactorlevel_low = self.w.create_rectangle(self.canvas_reactor_min_x,
                                                        ...
                                                        width=1, fill=GREEN_COLOR, outline=GREEN_COLOR) # (x=266, y=200, w=14, h=14)

        self.reactorlevel_high = self.w.create_rectangle(self.canvas_reactor_max_x,
                                                         ...
                                                         width=1, fill=GREEN_COLOR, outline=GREEN_COLOR) # (x=266, y=126, w=14, h=14)
    ...
    # This init function is called by the winhome auto button callback. It sets the interrupts for the reactor levels
    # and starts the adcloop function.
    # After some time (loading time), the control algorithm main function (init_fn) is called also (TODO: Since adcloop is also called and
    # itself can call the inits_fn this can potential cause some thread problems...)
    def inits(self):
        self.first_run = True
        self.iserror = False
        GPIO.add_event_detect(self.constructor.reactor_llow_pin, GPIO.BOTH, callback=self.l1_call, bouncetime=100)
        GPIO.add_event_detect(self.constructor.reactor_lhigh_pin, GPIO.BOTH, callback=self.l2_call, bouncetime=100)
        self.adcloop()
        self.master.after(self.LOADING_TIME, self.inits_fn)

        # Initiate the status flag. This flag is responsible for identifying the current system status.
        # Possible states are: 
    ...
    # This method runs as a loop by using the .after method. Its main purpose is to read the buffer from the ADC converter.
    # Additionally tasks are done such as updating the GUI and implementing an interrupt algorithm to respond to feed tk level
    # changes.  
    def adcloop(self):
        # This check assures that the ADC loop only runs when we are in the auto mode.
        if self.constructor.global_focus != 'auto':
            return
        # This check assures we only run the algorithm when there is no pop-up transition happening. This is to 
        # smooth out the pop transition (purely a cosmetic aspect). TODO: Can this be problematically?
        if not self.pop_running:
            # try: TODO: Catching exceptions is not needed here?
            ...
            # Split the result using some keywords. TODO - Change keywords in the ADC sketch
            ...
            # Convert the reading from eTape to canvas coordinates for displaying feed tk level
            ...
            # Convert reading from eTape to a percent value and display on label
            ...
            self.feed_level_lst.append(self.level_percent) # Append the new value
            ...
            if all(ii > (100 - self.constructor.db['feed_tk_level_params'][4]) for ii in self.feed_level_lst):
            ...
            elif all(ii > self.constructor.db['feed_tk_level_params'][3] for ii in self.feed_level_lst):
            ...
            elif all(ii < self.constructor.db['feed_tk_level_params'][4] for ii in self.feed_level_lst):
            ...
            elif all(ii < self.constructor.db['feed_tk_level_params'][2] for ii in self.feed_level_lst):
            ...
            elif all(ii > self.constructor.db['opt_feedtk_level'][3] and ii < self.constructor.db['feed_tk_level_params'][3] for ii in self.feed_level_lst):
            ...
            elif all(ii > self.constructor.db['feed_tk_level_params'][2] and ii < self.constructor.db['opt_feedtk_level'][3] for ii in self.feed_level_lst):
            ...
            if self.level_percent > self.constructor.db['feed_tk_level_params'][3] or self.level_percent < self.constructor.db['feed_tk_level_params'][2]:
            ...
            if self.l1_open:
                self.w.itemconfigure(self.reactorlevel_low, fill=RED_COLOR, outline=RED_COLOR)
                if self.l2_open:
                    self.lbl_reactor['image'] = self.ima_reactor00
                else:
                    self.lbl_reactor['image'] = self.ima_reactor01
            else:
                self.w.itemconfigure(self.reactorlevel_low, fill=GREEN_COLOR, outline=GREEN_COLOR)
                if self.l2_open:
            ...
            if self.l2_open:
                self.w.itemconfigure(self.reactorlevel_high, fill=GREEN_COLOR, outline=GREEN_COLOR)
                if self.l1_open:
                    self.lbl_reactor['image'] = self.ima_reactor00
                else:
                    self.lbl_reactor['image'] = self.ima_reactor10
            else:
                self.w.itemconfigure(self.reactorlevel_high, fill=RED_COLOR, outline=RED_COLOR)
                if self.l1_open:
                    self.lbl_reactor['image'] = self.ima_reactor01
                else:
                    self.lbl_reactor['image'] = self.ima_reactor11
    ...
    # Control algorithm function. This method is responsible for all the automatic control
    def inits_fn(self):
        ...
        # IDLE -------------------------------------------------------------------
        # If status is IDLE - Idle state is entered upon emptying stage completion
        if self.constructor.db['status_flag'] == 'idle':

            # First, test the lfeed value (a percent value). If the value is not good return from the function
            # and issue an error message (TODO: This may not be needed)
            if lfeed < 0 or lfeed > 100:
                lbl = 'ERROR-101'
                txt = 'The feed tank level sensor may not be working properly. Cannot proceed.'
            ...
            # If feed tank has a low low alarm
                ...
                if l1_open and l2_open:
                    lbl = 'WARNING-101'
                    txt = 'Feed tank has an unexpected low-low level alarm. Waiting for level to start sterilization.'
                ...
                # Reactor is not empty
                    ...
                    if self.constructor.db['enable_semi_auto'][1]:
                        if self.proceed_1:
                            self._hide_transient_buttons()
                            lbl = 'WARNING-102'
                            txt = 'Attempted to start filling reactor. Feed tank has a low-low level alarm. Reactor should be empty. User chose to proceed.'
                            self.update_info_box(lbl, txt)
                            self._hide_transient_buttons()
                        else:
                            self._hide_transient_buttons()
                            lbl = 'WARNING-102'
                            txt = 'Attempted to start filling reactor. Feed tank has a low-low level alarm. Reactor should be empty. Proceed anyway?'
                            self.update_info_box(lbl, txt)
                            self.proceed_id = 1
            ...
            # If feed tank has a low alarm
                ...
                if l1_open and l2_open:
                    lbl = 'WARNING-104'
                    # txt = 'Attempted to start filling reactor. Feed tank has a low level alarm. Waiting for level to start sterilization.'
                    txt = 'Feed tank has a low level alarm. Waiting for level to start sterilization.'
                    self.update_info_box(lbl, txt)
                ...
                # Reactor is not empty
                    ...
                    if self.constructor.db['enable_semi_auto'][1]:
                        if self.proceed_2:
                            lbl = 'WARNING-105'
                            txt = 'Attempted to start filling reactor. Feed tank has a low level alarm. Reactor should be empty. User chose to proceed.'
                            self.update_info_box(lbl, txt)
                        else:
                            lbl = 'WARNING-105'
                            txt = 'Attempted to start filling reactor. Feed tank has a low level alarm. Reactor should be empty. Proceed anyway?'
                            self.update_info_box(lbl, txt)
                            self.proceed_id = 2
                            self._show_proceed_button()
                    # If semi-auto mode is disabled
            ...
            # If feed tank level is between llow and lopt
            elif lfeed > self.constructor.db['feed_tk_level_params'][2] and lfeed <= self.constructor.db['opt_feedtk_level'][3]:
                self._hide_transient_buttons()

                # If the reactor is empty (NORMAL CONDITION)
                if l1_open and l2_open:
                    lbl = 'NOTE'
                    txt = 'Not enough level in the feed tank to start sterilization. Minimum level is defined as {0:.1f}%. Waiting for level to start sterilization.'.format(self.constructor.db['opt_feedtk_level'][3])
                    self.update_info_box(lbl, txt)
                    self._hide_transient_buttons()
                    # # If semi-auto mode is enabled
                    # if self.constructor.db['enable_semi_auto'][1]:
                    #     lbl = 'NOTE'
                    #     txt = 'The feed tank level is less than the defined minimum level ({0:.1f}%) to start sterilization. Do you want to start sterilization anyway?'.format(self.constructor.db['opt_feedtk_level'][3])
                    #     self.update_info_box(lbl, txt)
                ...
                # Reactor is not empty
                    ...
                    # If semi-auto mode is enabled
                    # TODO: Think about this... Should steri start with a small volume in the reactor?
                    if self.constructor.db['enable_semi_auto'][1]:
                        lbl = 'WARNING-107'
                        txt = 'The feed tank level is less than the defined minimum level ({0:.1f}%). The reactor is not empty. Proceed to sterilization anyway?'.format(self.constructor.db['opt_feedtk_level'][3])
                        self.update_info_box(lbl, txt)
                        self._show_steri_button()
                    # If semi-auto mode is disabled
                    else:
            ...
            # If feed level is between lopt and lhigh (NORMAL CONDITION):
            elif lfeed > self.constructor.db['opt_feedtk_level'][3] and lfeed <= self.constructor.db['feed_tk_level_params'][3]:
                self._hide_transient_buttons()

                # If the reactor is empty (NORMAL CONDITION)
                if l1_open and l2_open:
                    lbl = 'NOTE'
                    txt = 'Filling reactor. Sterilization will start after filling. Please do not power-off.'
                    self.update_info_box(lbl, txt)
                    # GPIO
                ...
                # Reactor is not empty
                    ...
                    if self.constructor.db['enable_semi_auto'][1]:
                        lbl = 'WARNING-109'
                        txt = 'Attempted to start filling reactor. The reactor is not empty as it should be. Proceed to sterilization anyway?'
                        self.update_info_box(lbl, txt)
            ...
            # If feedtk has an high-high alarm
                ...
                lbl = 'ERROR-114'
                txt = 'Feed tank has a high-high level alarm. Cannot proceed. Please make sure no more liquid enters the machine.'
                self.update_info_box(lbl, txt)
            ...
            # If feedtk has an high alarm
                ...
                if l1_open and l2_open:
                    lbl = 'WARNING-111'
                    txt = 'The feed tank has a high level alarm. The feed flowrate to the machine may be too high. Starting filling reactor.'
                    self.update_info_box(lbl, txt)
                    # GPIO
                    self.turn_feedpmp_on()
                    self.constructor.db['status_flag'] = 'filling'
                ...
                # Reactor is not empty
                    ...
                    if self.constructor.db['enable_semi_auto'][1]:
                        lbl = 'WARNING-112'
                        txt = 'Attempted to start filling reactor. The reactor is not empty. Feed tank has a high level alarm. Do you want to proceed anyway?'
                        self.update_info_box(lbl, txt)
                        self._show_proceed_button()
                    # If semi-auto mode is disabled
                    else:
        ...
        # If status is FILLING
            ...
            # TODO: This condition may cause problems
                ...
                self._hide_transient_buttons()
                lbl = 'ERROR-201'
                txt = 'Unexpected system state detected. Feed tank has a low-low level alarm. Cannot proceed.'
                self.update_info_box(lbl, txt)
                self.error_procedure()
            ...
            # If feed tank has a low alarm (NORMAL - stop filling)
                ...
                # The reactor is empty - At this stage the reactor and the feed tk should not be both empty
                if l1_open and l2_open:
                    lbl = 'ERROR-202'
                    txt = 'Unexpected system state detected. Please check for leakages. Cannot continue.'
                    self.update_info_box(lbl, txt)
                    self.error_procedure()
                ...
                # Reactor is not empty (NORMAL CONDITION)
                    ...
                    # lbl = 'NOTE'
                    # txt = 'Starting sterilization protocol. Please do not power-off. Sterilization time is defined as {0:.1f}min.'.format(self.constructor.db['steri_time'][3])
                    # self.update_info_box(lbl, txt)
            ...
            # If feed tank level is between llow and lopt
                ...
                # Reactor has no low level but has a high level (IMPOSSIBLE)
                # TODO: Deal with errors. P.e. show a popup and freeze the entire ui
                elif l1_open and not l2_open:
                    lbl = 'ERROR-205'
                    self.rx_levels_error(lbl)
            ...
            # If feedtk has a high level alarm
                ...
                if l1_open and l2_open:
                    if self.first_run:
                        lbl = 'WARNING-204'
                        txt = 'Feed tank has a high level alarm. The feed flowrate to the machine may be too high.'
                        self.update_info_box(lbl, txt)
                        # GPIO
                        self.turn_feedpmp_on()
                        # self.constructor.save_db()
        ...
        # If status is STERI
            ...
            # If feed tank has a low alarm
                ...
                if l1_open and l2_open:
                    lbl = 'ERROR-302'
                    txt = 'Unexpected system state detected. Reactor seems to be empty. Cannot continue.'
                    self.update_info_box(lbl, txt)
                    self.error_procedure()
            ...
            # If feedtk has an high-high alarm
            # TODO: cleanGPIO and signal the error state
            elif lfeed >= 100 - self.constructor.db['feed_tk_level_params'][4]:
                self._hide_transient_buttons()
                lbl = 'ERROR-308'
                txt = 'Feed tank has a high-high level alarm. Cannot proceed. Please make sure no more liquid enters the machine.'
        ...
        # If status is EMPTYING
            ...
            # First, test the lfeed value (a percent value). If the value is not goo from the function
            # and issue an error message
            if lfeed < 0 or lfeed > 100:
                lbl = 'ERROR-400'
                txt = 'The feed tank level sensor may not be working properly. Cannot proceed.'
            ...
            # If feed tank has a low alarm
                ...
                if l1_open and l2_open:
                    ...
                    self.clean_GPIO()
                    # Run itself to see if we have enough liquid volume in feedtk to start over
                    self.inits_fn()

                # # Reactor is not empty (NORMAL CONDITION)
                if not l1_open and l2_open:
                    lbl = 'NOTE'
                    txt = 'Sterilization completed. Emptying reactor. Please do not power-off.'
                    self.update_info_box(lbl, txt)
                    # GPIO
                    self.turn_outpmp_on()
                    self.turn_uv_on()
            ...
            # If feed tank level is between llow and lopt
                ...
                if l1_open and l2_open:
                    self.constructor.db['status_flag'] = 'idle'
                    self.clean_GPIO() # TODO: Adjust the cleanGPIO() method
                    self.inits_fn()

                # Reactor is not empty (NORMAL CONDITION)
                # TODO: Does not this condition repeat itself until something happens with the
                # reactor's level sensors?
                elif not l1_open and l2_open:
                    lbl = 'NOTE'
                    txt = 'Sterilization completed. Emptying reactor. Please do not power-off.'
                    self.update_info_box(lbl, txt)
            ...
            # If feed level is between lopt and lhigh:
                ...
                if l1_open and l2_open:
                    self.constructor.db['status_flag'] = 'idle'
                    self.clean_GPIO()
                    self.inits_fn() # TODO: Why call this method (inits_fn()) here?
            ...
            # If feedtk has an high-high alarm
            # TODO: This is the best solution? Stop the system when the feedtk gets an highhigh alarm?
            # Why not give a warning instead? The inlet to the machine must have a sensor anyway...
            elif lfeed >= 100 - self.constructor.db['feed_tk_level_params'][4]:
                self._hide_transient_buttons()
                lbl = 'ERROR-408'
                txt = 'Feed tank has a high-high level alarm. Cannot proceed. Please make sure no more liquid enters the machine.'
    ...
    def steri_procedure(self):
        ...
        self._hide_transient_buttons()
        lbl = 'NOTE'
        txt = 'Performing sterilization. Stage 1: Warming ozone generator. Please do not power-off.'
        self.update_info_box(lbl, txt)
    ...
    def steri_loop1(self):
        ...
        if self.constructor.db['status_flag'] == 'steri' and self.steri_stage == 'loop1':
            now = time.time()
            ...
            if time_passed > self.constructor.db['warmup_time'][3]:
                ...
                lbl = 'NOTE'
                txt = 'Performing sterilization. Stage 2: Sterilization time is defined as {0:.1f}min.  Please do not power-off.'.format(self.constructor.db['steri_time'][3])
                self.update_info_box(lbl, txt)
    ...
    def steri_loop2(self):
        ...
        if self.constructor.db['status_flag'] == 'steri' and self.steri_stage == 'loop2':
            now = time.time()
            time_passed = (now - self.steri_start_time) / 60
            if time_passed >= self.constructor.db['steri_time'][3]:
                ...
                lbl = 'NOTE'
                txt = 'Performing sterilization. Stage 3: Settling. Please do not power-off.'
                self.update_info_box(lbl, txt)
    ...
    def steri_loop3(self):
        ...
        if self.constructor.db['status_flag'] == 'steri' and self.steri_stage == 'loop3':
            now = time.time()
            time_passed = (now - self.steri_start_time) / 60
    ...
    def update_ui(self):
        ...
        if self.toggle_feedpmp == 'on':
            ...
            self.w.itemconfigure('filling_poly', outline=LIGHT_BLUE_COLOR)
        else:
        ...
        if self.toggle_recirpmp == 'on':
            ...
            if self.toggle_o2 == 'on':
                ...
                self.w.itemconfigure(self.canvas_o2_recir, image=self.ima_o2_recir01)
            self.w.itemconfigure('recirpmp_poly', outline=LIGHT_BLUE_COLOR)
        else:
        ...
        if self.toggle_outpmp == 'on':
            ...
            self.w.itemconfigure('out_poly', outline=LIGHT_BLUE_COLOR)
        else:
    ...
    # This method changes the proceed flags in order to update the ui in case the user chooses to proceed when facing a warning.
    # For instance, after the user chooses to proceed the proceed button must be unpacked and the info string changed to reflect
    # the user option to proceed. (TODO: The proceed_# flags must be set to false when entering a new state.)
    ...
    def lbl_feedpmp_call(self, event):
        # if self.constructor.global_focus != 'auto':
        #     return
        # self.pop_running = True
        # self.constructor.pop_feedpmpapp.update_pop()
        # self.constructor.anim_pop(event, self.constructor.pop_feedpmppanel_l, self.master, self)
        # self.master.after(1200, self.set_pop_flag)
    ...
    def canvasbind(self, event):
        if event.x > 28 and event.x < 73 and event.y > 4 and event.y < 60:
            ...
            self.pop_recirpmp(event)
        elif event.x > 343 and event.x < 381 and event.y > 12 and event.y < 100:
            self.pop_uv(event)
        elif event.x > 132 and event.x < 176 and event.y > 226 and event.y < 269:
            self.pop_feedpmp(event)
        elif event.x > 223 and event.x < 295 and event.y > 96 and event.y < 269:
            self.pop_reactor(event)
        elif event.x > 340 and event.x < 385 and event.y > 226 and event.y < 269:
            self.pop_outpmp(event)
    ...
    def pop_o2(self, event):
        ...
        self.constructor.anim_pop(event, self.constructor.pop_o2panel, self.master, self)

    def pop_o3(self, event):
        if self.constructor.global_focus != 'auto':
            return
        self.constructor.pop_o3app.update_pop()
        self.constructor.anim_pop(event, self.constructor.pop_o3panel, self.master, self)

    def pop_feedpmp(self, event):
        if self.constructor.global_focus != 'auto':
            return
        self.constructor.pop_feedpmpapp.update_pop()
        self.constructor.anim_pop(event, self.constructor.pop_feedpmppanel, self.master, self)

    def pop_recirpmp(self, event):
        if self.constructor.global_focus != 'auto':
            return
        self.constructor.pop_recirpmpapp.update_pop()
        self.constructor.anim_pop(event, self.constructor.pop_recirpmppanel, self.master, self)

    def pop_uv(self, event):
        if self.constructor.global_focus != 'auto':
            return
        self.constructor.pop_uvapp.update_pop()
        self.constructor.anim_pop(event, self.constructor.pop_uvpanel, self.master, self)

    def pop_reactor(self, event):
        if self.constructor.global_focus != 'auto':
            return
        self.constructor.pop_reactorapp.update_pop()
        self.constructor.anim_pop(event, self.constructor.pop_reactorpanel, self.master, self)

    def pop_outpmp(self, event):
        if self.constructor.global_focus != 'auto':
            return
        self.constructor.pop_outpmpapp.update_pop()
        self.constructor.anim_pop(event, self.constructor.pop_outpmppanel, self.master, self)

    def pop_feedtk(self, event):
        if self.constructor.global_focus != 'auto':
            return
        self.constructor.pop_feedtkapp.update_pop(self)
        self.constructor.anim_pop(event, self.constructor.pop_feedtkpanel, self.master, self)
    ...
    def clean_up(self):
        ...
        self.linfo_kw.config(text=self.LINFO_KW_ISTRING, fg=FG_COLOR)
    ...
    def idle_rx_not_empty(self, lbl):
        self.clean_GPIO()
        txt = 'Unexpected system state detected. Reactor is not empty as it should be at this point. Cannot proceed.'
        self.update_info_box(lbl, txt)
        self.error_procedure()
    ...
    def idle_rx_full(self, lbl1, lbl2):
        ...
        if self.constructor.db['enable_semi_auto'][1]:
            lbl = lbl1
            txt = 'Unexpected system state detected. The reactor should not be full at this stage. Proceed to sterilization anyway?'
            self._show_steri_button()
        else:
            lbl = lbl2
            txt = 'Unexpected state detected during idle. The reactor should not be full at this stage. Cannot proceed.'
            self.error_procedure()
        self.update_info_box(lbl, txt)

    def rx_levels_error(self, lbl):
        self.clean_GPIO()
        txt = 'Reactor level sensors are not working properly. Cannot proceed. Please check the sensors and try again.'
        self.update_info_box(lbl, txt)
        self.error_procedure()
    ...
    def filling_rx_full(self, lbl1, lbl2):
        ...
        if self.constructor.db['enable_semi_auto'][1]:
            ...
            lbl = lbl2
            txt = 'Unexpected state detected during filling. The reactor should not be full at this stage. Cannot proceed.'
            self.update_info_box(lbl, txt)
            self.error_procedure()
    ...
    def steri_rx_full(self, lbl1, lbl2):
        ...
        if self.constructor.db['enable_semi_auto'][1]:
            lbl = lbl1
            txt = 'Unexpected system state detected. The reactor should not be full at this stage. Cannot proceed.'
            self.update_info_box(lbl, txt)
            self.error_procedure()
        else:
            lbl = lbl2
            txt = 'Unexpected state detected during sterilization. The reactor should not be full at this stage. Cannot proceed.'
            self.update_info_box(lbl, txt)
            self.error_procedure()
    ...
    def turn_feedpmp_off(self):
        ...
        if GPIO.input(self.constructor.feedpmp_pin) == self.constructor.db['OFF']:
            ...
            if self.start_feedpmp_counter != 0:
                ...
                elapsed_time_min = int((self.stop_feedpmp_counter - self.start_feedpmp_counter) / 60)
    ...
    def turn_recirpmp_on(self):
        if GPIO.input(self.constructor.recirpmp_pin) == self.constructor.db['ON']:
            print('Attempted to start recir pump when it was already turned on.')
        else:
    ...
    def turn_recirpmp_off(self):
        if GPIO.input(self.constructor.recirpmp_pin) == self.constructor.db['OFF']:
            print('Attempted to stop the recir pump when it was already turned off.')
        else:
            ...
            if self.start_recirpmp_counter != 0:
                ...
                elapsed_time_min = int((self.stop_recirpmp_counter - self.start_recirpmp_counter) / 60)
    ...
    def turn_outpmp_off(self):
        if GPIO.input(self.constructor.outpmp_pin) == self.constructor.db['OFF']:
            print('Attempted to stop the out pump when it was already turned off.')
        else:
            ...
            if self.start_outpmp_counter != 0:
                ...
                elapsed_time_min = int((self.stop_outpmp_counter - self.start_outpmp_counter) / 60)
    ...
    def turn_o3_off(self):
        if GPIO.input(self.constructor.o3_pin) == self.constructor.db['OFF']:
            ...
            if self.start_o3_counter != 0:
                ...
                elapsed_time_min = int((self.stop_o3_counter - self.start_o3_counter) / 60)
    ...
    def turn_uv_off(self):
        if GPIO.input(self.constructor.uv_pin) == self.constructor.db['OFF']:
            ...
            if self.start_uv_counter != 0:
                ...
                elapsed_time_min = int((self.stop_uv_counter - self.start_uv_counter) / 60)
